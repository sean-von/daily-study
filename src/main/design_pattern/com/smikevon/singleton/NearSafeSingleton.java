...
/**
 * @description: 近似线程安全的单例实现，但仍有不安全的时候
 * @author     : fengxiao
 * @date       : 2014年10月27日 下午2:42:03
 */
public class NearSafeSingleton {

	private static NearSafeSingleton instance = null;

	private NearSafeSingleton(){
	}

	public static NearSafeSingleton getInstance(){
		if(instance == null){
			synchronized (instance) {
				if(instance == null){
					//在Java指令中创建对象和赋值操作是分开进行的 ，因此仍有可能出现线程不安全的情况，具体解释看下面
					instance = new NearSafeSingleton();
				}
			}
		}
		return instance;
